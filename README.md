...
- 微信真实手机号授权走 `bindPhoneNumber`；Demo 手动输入测试手机号走 `bindDemoPhoneNumber`。
...
3. 首次购买会出现手机号授权，授权后继续下单。
   - 如果当前小程序主体或能力配置暂时无法获取微信手机号，可在 Demo 手动输入框填写测试手机号。
4. 点击确认模拟支付。
5. 在“我的券”查看电子券和二维码。
...
### 真机点击绑定手机号仍提示未获取到手机号授权

这通常不是代码问题，而是当前小程序账号暂时没有真实手机号授权能力。微信手机号能力需要用户主动点击授权按钮，并且小程序主体、认证、隐私协议和能力配置都满足要求。Demo 阶段可以先使用“Demo 手动绑定”输入测试手机号，继续跑通购买、发券、核销和商家查询流程。真实上线前再用商家主体开通并测试微信手机号授权。
